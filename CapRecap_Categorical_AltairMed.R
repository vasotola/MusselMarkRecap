...
dis<-read.csv("MedDischargeDummy_ALT.csv",header=F)
...
discharge <- array(data=dis,dim=c(120,4,2))
discharge[1:10,1:4,1]
...
model {
  ...
  # Linear predictors of phi and gamma
    ...
    for (t in 1:(n.years-1)){
       ...
       logit(phi[i,t]) <- alpha.phi + beta.1 * discharge[i,t,1] + beta.2 * discharge[i,t,2]
...
pars <- c('pstar', 'p', 'alpha.phi','beta.1','beta.2',
          'alpha.gamma')
