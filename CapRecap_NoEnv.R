...
d<-read.csv("SS_CH_QUPE.csv",header=F)
...
# phi adjusted for probability of losing both PIT
    for (t in (first[i]+1):n.years){
      ##mu1[i,t] <- z[i,t-1] * phi[t-1]##
      mu1[i,t] <- z[i,t-1] * (phi[t-1] * 0.947)
...
#rdt.alt.qupe.adj<-rdt
#rdt.alt.quho.adj<-rdt
#rdt.ss.qupe.adj<-rdt
rdt.ss.quho.adj<-rdt

#save.image("FinalCapRecap_PIT.RData")
#load("FinalCapRecap_PIT.RData")

save.image("FinalCapRecap_PIT_ADJ.RData")
#load("FinalCapRecap_PIT.RData")
...
#altair only because we ran out of PIT tags once so we can't calculate
...
#SS
prob<-19/777*(1-19/777)
prob

## prob of losing both tags
