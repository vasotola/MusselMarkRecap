...
d<-read.csv("SS_QUHO_floy_CH.csv",header=F)
#d<-read.csv("Alt_QUHO_floy_CH.csv",header=F)
...
# phi adjusted for probability of losing both PIT and floy
    for (t in (first[i]+1):n.years){
      ##mu1[i,t] <- z[i,t-1] * phi[t-1]
      mu1[i,t] <- z[i,t-1] * (phi[t-1] * 0.99873)
...
# adjusted for probability of losing both PIT and floy
    for(t in 1:n.years){
        Nin[t] <- (sum(ch[,t])/pstar[t]) * 0.99873
    }
...
save.image("FinalCapRecap_Grub_ADJ.RData")
